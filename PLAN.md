...
3b. **Fulfill Engine auto-fulfillment BUILT 2026-07-20** (`api/_lib/fulfillengine.js` + webhook block + admin validate-then-submit "Send to Fulfill Engine"): FE items submit via the FE merchant API as campaignProductId + color/size ('One Size' omitted), idempotent by order id. **Inert until `FE_API_KEY` is set in Vercel** (FE dashboard → Configuration → API keys). To finish: set the key; on the hat order run Recover address → Send to Fulfill Engine; complete the Market Bag's address in the Printify UI; run `supabase-fe-order-id.sql` (backlink column — code is fail-soft without it).
