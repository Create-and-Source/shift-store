...
1d. **Small cleanups**: run `supabase-fulfillment-error.sql` (fail-soft until then — stamps just log); ~~run `supabase-fe-order-id.sql`~~ DONE 2026-07-20 (via platform pg-meta API — the SQL editor UI never mounts in automation; see the settlement section); ~~delete test auth user `shift-signup-test-0720@…`~~ DONE 2026-07-20 (verified zero linked customers/orders first); cancel/delete leftover test orders in /dashadmin + refund test payments in Stripe (both accounts, her call); pay/verify the Baby T + hat + bag productions.
